class postfix::config (

	$nodename = hiera('nodename'),
	$configuration_directory = hiera('configuration_directory'),
	$service_name = hiera('postfix::service::service_name'),
	$db_files = hiera('db_files'),
    
	) inherits postfix {


  $config_files.each |String $file| {
    file { "${configuration_directory}${file}":
      content => file("postfix/${nodename}/${configuration_directory}${file}"),
      require => File["${configuration_directory}"],
    }
  }

  $access_files.each |String $file| {
    file { "${configuration_directory}${file}":
      content => file("postfix/${nodename}/${configuration_directory}${file}"),
      require => File["${configuration_directory}"],
    }
  }

  file { "${configuration_directory}${postfix_files_directory}":
    ensure => 'directory',
    mode    =>  '0755',
  }

  $postfix_files_content.each |String $file| {
    file { "${configuration_directory}${postfix_files_directory}}${file}":
      content => file("postfix/${nodename}${configuration_directory}${postfix_files_directory}}${file}"),
      require => File["${configuration_directory}${postfix_files_directory}"],
    }
  }

  file { "${configuration_directory}${sasl_files_directory}":
    ensure => 'directory',
    mode    =>  '0755',
  }

  $sasl_files.each |String $file| {
    file { "${configuration_directory}${sasl_files_directory}${file}":
      content => file("postfix/${nodename}${configuration_directory}${sasl_files_directory}${file}"),
      require => File["${configuration_directory}${sasl_files_directory}"],
    }
  }

  $db_files.each |String $dbfile| {
    file { "${configuration_directory}${dbfile}":
      audit => 'content',
      require => File["${dbfile}"],
    }
  }

  $db_files.each |String $dbfile| {
    exec {"postfix.recreate.${dbfile}":
      command => "/usr/sbin/postmap ${configuration_directory}/${dbfile}",
      path        => ['/usr/bin', '/usr/sbin'],
      subscribe   => File["${configuration_directory}${dbfile}"],
      refreshonly => true,
    }
  }

  $db_files.each |String $dbfile| {
    exec {"postfix.create.${dbfile}":
      command => "/usr/sbin/postmap ${configuration_directory}/${dbfile}", 
      path        => ['/usr/bin', '/usr/sbin'],
      onlyif =>  "test ! -f ${configuration_directory}/${dbfile}.db",
    }
  }

}
